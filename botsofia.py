...
def obtener_resultados_winbig():
    ...
    try:
        ...
        # 1. Ignorar y eliminar completamente la sección de "ÚLTIMOS SALIDOS" o equivalentes
        for elem in soup.find_all(['div', 'section', 'article', 'aside', 'header']):
            texto_elem = elem.get_text(" ", strip=True).upper()
            if "ÚLTIMOS SALIDOS" in texto_elem or "RESULTADOS DE HOY" in texto_elem:
                elem.decompose()

        # 2 y 5. Leer únicamente bloques normales de forma dinámica
        ...
        for tarjeta in tarjetas:
            ...
            if "ÚLTIMOS SALIDOS" in texto_tarjeta:
                continue
            ...
            # 4, 6 y 7. Sin fallback fijo, detección automática y descarte si no hay seguridad
            ...
            for slot in slots_sorteo:
                ...
                if "PENDIENTE" in texto_slot:
                    resultados_por_loteria[nombre_loteria_ind].append((hora, "⏳ Pendiente", ""))
                else:
                    ...
                    if match_res:
                        ...
                        resultados_por_loteria[nombre_loteria_ind].append((hora, resultado, resultado.split("-")[0].strip().zfill(2)))
...
def obtener_animales_salidos_actuales():
    salidos = set()
    try:
        resultados = obtener_resultados_winbig()
        for _, slots in resultados.items():
            for _, _, numero in slots:
                if numero:
                    salidos.add(numero)
    except Exception as e:
...
def verificar_y_enviar_resultados_individuales():
    ...
    try:
        ...
        for nombre_loteria_ind, slots in resultados.items():
            for hora, resultado, numero in slots:
                if resultado == "⏳ Pendiente" or not numero:
                    continue
...
def cmd_resumen(message):
    try:
        ...
        for loteria, slots in resultados.items():
            resumen_por_loterias[loteria] = []
            for hora, resultado, _ in slots:
                if resultado == "⏳ Pendiente":
                    resumen_por_loterias[loteria].append(f"• {hora} ➔ ⏳ Pendiente")
                else:
                    resumen_por_loterias[loteria].append(f"• {hora} ➔ {resultado}")
