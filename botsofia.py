...
def verificar_y_enviar_resultados_individuales():
    ...
    try:
        ...
        # BÚSQUEDA ROBUSTA GENERALIZADA: Busca cualquier bloque o tarjeta contenedora posible
        tarjetas = soup.find_all(['div', 'article', 'section', 'tr', 'li', 'ul'], class_=True)
        if not tarjetas:
            tarjetas = soup.find_all(['div', 'article', 'section', 'tr', 'li', 'ul'])
        ...
        for tarjeta in tarjetas:
            ...
            # Buscamos si el bloque contiene una hora y un resultado estilo "XX - Animal"
            match_h = re.search(r'\b(\d{1,2}:\d{2}\s*(?:AM|PM))\b', texto_tarjeta, re.IGNORECASE)
            match_res = re.search(r'(\d{1,2}\s*-\s*[A-ZÁÉÍÓÚÑa-zñáéíóú]+(?:\s+[A-ZÁÉÍÓÚÑa-zñáéíóú]+)?)', texto_tarjeta)
            ...
            # Intentar deducir el nombre de la lotería desde el bloque o textos cercanos
            ...
            # Si hay palabras claves comunes en títulos
